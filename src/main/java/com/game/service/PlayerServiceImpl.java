...
public class PlayerServiceImpl implements PlayerService{
    ...
    public Player createPlayer(Player playerRequired) {
        if (playerRequired == null
                || playerRequired.getName() == null
        ...
        if (invalidParameters(playerRequired)) return null;

        if (playerRequired.isBanned() == null) playerRequired.setBanned(false);

        setLevelAndExpUntilNextLevel(playerRequired);
    ...
    public Player updatePlayer(Long id, Player playerRequired) {
        ...
        if (playerRequired.getName() != null) responsePlayer.setName(playerRequired.getName());
        if (playerRequired.getTitle() != null) responsePlayer.setTitle(playerRequired.getTitle());
        if (playerRequired.getRace() != null) responsePlayer.setRace(playerRequired.getRace());
        if (playerRequired.getProfession() != null) responsePlayer.setProfession(playerRequired.getProfession());
        if (playerRequired.getBirthday() != null) responsePlayer.setBirthday(playerRequired.getBirthday());
        if (playerRequired.isBanned() != null) responsePlayer.setBanned(playerRequired.isBanned());
        if (playerRequired.getExperience() != null) responsePlayer.setExperience(playerRequired.getExperience());
    ...
    public Specification<Player> nameFilter(String name) {
        ...
        /*EntityManager entityManagerFactory = (EntityManager) Persistence.createEntityManagerFactory("ru.easyjava.data.jpa.hibernate");
        EntityManager em = entityManagerFactory.createEntityManager();
        em.getTransaction().begin();
        CriteriaBuilder cb = em.getCriteriaBuilder();
*/

        //return playerRepository.findPlayersByNameContains(name, pageable);//(Player) getPlayersList().stream().filter(x-> x.getName().equals(name));
    }
